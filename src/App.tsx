...
const floor10 = [
  ...
  return (
    <div style={{ width: `${width}px`, position: "relative" }}>
      ...
      {/* SVG Overlay 30th to 41st floor */}
      <svg
        viewBox="0 0 800 472"
      ...
      >
        ...
        ].flatMap(({ floorNum, polygons }) =>
          polygons.map((poly, j) => {
            const isExtraPoint = j >= 3;
            return (
              <polygon
                key={`${floorNum}-${j}`}
                points={poly.points}
                fill={!isExtraPoint && hoveredGroup === floorNum ? "rgba(38, 221, 53, 0.55)" : "transparent"}
                stroke={!isExtraPoint && hoveredGroup === floorNum ? "gold" : "transparent"}
                strokeWidth="0.8"
